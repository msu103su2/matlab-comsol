...
for Index = 1:size(DN,2)
    ...
    if(~sql.IsFieldExsist(Tablename, 'Q'))
        sql.AddField(Tablename, 'Q', 'json')
    end
    ...
    for i = 1:NumberOfDevices
        ...
        jsontxtForQ = jsonencode(real(result.QualityFactor));
        
        sql.UpdateJSONField('Eigenfreq', jsontxt, DeviceSN);
        sql.UpdateJSONField('Q', jsontxtForQ, DeviceSN);
