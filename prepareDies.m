...
DiesParams = generateDiesfromOngoingData('PSO\data_dataRE_1mm_2nd_ongoing.mat',[5:10],DiesParams, WaferSN,conn,Tablename,2);
DiesParams = generateDiesfromOngoingData('PSO\data_dataRE_2mm_2nd_ongoing.mat',[12:17],DiesParams,WaferSN,conn,Tablename,1);
DiesParams = generateDiesfromOngoingData('PSO\matlab.mat',[20:25],DiesParams, WaferSN,conn,Tablename,2);
DiesParams = generateDiesfromOngoingData('PSO\data_centertilt_2mm_2nd_ongoing.mat',[28:33],DiesParams,WaferSN,conn,Tablename,1);
...
GDSdir = sprintf('Z:\\User\\Shan\\Devices\\WaferSN_%s\\GDSfile',WaferSN);
ToCNST(DiesParams, Filename, 1, 11, WaferSN, CNSTdir, GDSdir);
...
function newDiesParams = generateDiesfromOngoingData(DataPath,Dienumbers,DiesParams,WaferSN,conn,Tablename, DesignedLength)

Ystep = 200;%um
Yrange = [-1900,1900];%um

load(DataPath);
[temp,Alldata.searchresult] = max([Alldata.SingleResults.cooperation]);
DieParams.Params = Alldata.params(Alldata.searchresult,1:2);
VPname = 'DL';
RVRL = -0.2;
RVRH = 0.2;
[DieParams,NumberOfDevices] = PrepareDie(DieParams, VPname, RVRL, RVRH, Ystep, Yrange, Dienumbers(1), WaferSN);
...
[temp,Alldata.searchresult] = max([Alldata.SingleResults.cooperation]);
VPname = 'UL';
Params = Alldata.params(Alldata.searchresult,1:2);
RVRL = Params{2}{1}.value-(Params{2}{7}.value+2*Params{2}{10}.value*tan(22.5*pi/180)+2*Params{2}{8}.value);
RVRL = max([-0.2, -RVRL]);
RVRH = 0.2;
clear Alldata;
